...
| --- | --- | --- | --- |
| 1 | [`EnumBasics.java`](EnumBasics.java) | Enum（列挙型） | 誤字があってもコンパイルが通るString定数管理 |
| 2 | [`GenericsBasics.java`](GenericsBasics.java) | ジェネリクス | 実行時にClassCastExceptionが発生するObject型の箱 |
| 3 | [`LambdaBasics.java`](LambdaBasics.java) | ラムダ式 | 同じ構造のforループを毎回書く冗長なコード |
| 4 | [`FunctionalInterfaces.java`](FunctionalInterfaces.java) | 関数型インターフェース | Predicate/Function/Consumer/Supplierの4種を体験 |
| 5 | [`HigherOrderFunctions.java`](HigherOrderFunctions.java) | 高階関数 | 条件ごとに別メソッドを量産する設計の限界 |
| 6 | [`InterfaceAndPolymorphism.java`](InterfaceAndPolymorphism.java) | インターフェース・ポリモーフィズム | 支払い方法が増えるたびにif文が増え続ける設計 |
| 7 | [`StaticAndAnnotation.java`](StaticAndAnnotation.java) | static・アノテーション | 状態を持つstaticメソッドが呼び出し順で結果が変わる |
...
> 「不明な状態」と「コンパイルエラー」の違いを確認したら、次は [`GenericsBasics.java`](GenericsBasics.java) で同じ「型安全」の考え方がどう使われるか見てみましょう。
...
> 「実行時エラー」を「コンパイル時エラー」に変える感覚をつかんだら、次は [`LambdaBasics.java`](LambdaBasics.java) で「振る舞いを変数に入れる」という新しい発想を体験しましょう。
...
Step1→Step4 の変化を確認したら、次は [`FunctionalInterfaces.java`](FunctionalInterfaces.java) でJavaが用意している標準の「振る舞いの型」を学びましょう。
...
> 4種のインターフェースを動かしたら、次は [`HigherOrderFunctions.java`](HigherOrderFunctions.java) で「振る舞いを引数に渡す」ことで重複コードがどう消えるかを体験しましょう。
...
> 「振る舞いを引数で渡す」感覚をつかんだら、次は [`InterfaceAndPolymorphism.java`](InterfaceAndPolymorphism.java) でその考え方がクラス設計にどう応用されるか見てみましょう。
...
> 新しい支払い方法を追加したとき呼び出し元が無変更だった理由を言葉で説明できたら、最後の [`StaticAndAnnotation.java`](StaticAndAnnotation.java) でよくあるミス（static 乱用）を体験しましょう。
...
1. [`EnumBasics.java`](EnumBasics.java) の `switch` 式から `default ->` の行をコメントアウトして実行してみましょう。
   コンパイルがどうなるかを確かめよう。`default` があることで「どんな安全性」が生まれているかを言葉で説明してみましょう。

2. [`GenericsBasics.java`](GenericsBasics.java) の `Box<String>` を `Box<Integer>` に変えた場合、エラーはどのタイミングで発生しますか？
   「実行時エラー」と「コンパイルエラー」のどちらになるかを実際に試して確かめましょう。

3. [`LambdaBasics.java`](LambdaBasics.java) の Step3（ラムダ式）を Step2（匿名クラス）の書き方に書き直してみましょう。
...
5. [`StaticAndAnnotation.java`](StaticAndAnnotation.java) の `@Override` を `@Overide`（スペルミス）に変えてコンパイルするとどうなりますか？
   確認したら元に戻しておきましょう。

6. [`InterfaceAndPolymorphism.java`](InterfaceAndPolymorphism.java) に新しい支払い方法 `ConveniencePayment`（コンビニ払い）を追加してみましょう。
